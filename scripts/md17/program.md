...
# Experimentation
In each experiment `n`, create a folder named `experiments/n` and copy `run.py` into that folder as `experiments/n/run.py`. Modify that file. Launch it with `start_experiment(n)`, which starts training for exactly **one epoch** in the background and returns immediately. Training auto-saves a checkpoint to `experiments/{n}/checkpoint.pt` and appends that epoch's errors to `experiments/{n}/metrics.jsonl`. Use `poll_experiment(n)` to check progress and retrieve output. Call `start_experiment(n)` again after it finishes to resume from the checkpoint and train another epoch.
...
3. **Decide**: should you continue training an existing experiment, or start a new one?
   - **Continue** if `val_force_error` / `val_energy_error` are still decreasing epoch-over-epoch. Call `start_experiment(n)` again — it automatically resumes from the last checkpoint.
...
4. If starting a new experiment: read the best-performing `experiments/{prev_n}/run.py`, write a modified copy to `experiments/{n}/run.py`, then call `start_experiment(n)`.
5. After calling `start_experiment(n)`, do not wait idly — analyse other experiments, plan the next one, or prepare its `run.py`. Periodically call `poll_experiment(n)` to check progress. Once it reports finished, call `read_metrics(n)` to get the updated trend and decide whether to keep training or branch.
